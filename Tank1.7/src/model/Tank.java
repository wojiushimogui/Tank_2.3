...
//				break;
		...
		private void produceEnemyTanks(int totalNum) {
			//产生totalNum个敌方坦克
			for(int i=0;i<totalNum;i++){
				int x = (int)(1+Math.random()*tc.GAME_WIDTH);
				int y = (int)(1+Math.random()*tc.GAME_HEIGHT);
				Tank enemy = new Tank(x,y,false,this.tc);
				tc.getEnemyTanks().add(enemy);
			}
		}
		...
		public Missile fire(){
			...
			Missile ms = new Missile(x,y,this.ptDir,this.tc);
			return ms;
		}
		/*
		 * 函数功能：得到坦克所在位置的矩形框
